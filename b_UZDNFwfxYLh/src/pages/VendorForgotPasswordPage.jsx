...
import { useRequestVendorPasswordOtpMutation, useResetVendorPasswordMutation, useVerifyVendorPasswordOtpMutation } from "@/features/api/apiSlice"
...
export default function VendorForgotPasswordPage() {
  ...
  const [requestOtp, requestState] = useRequestVendorPasswordOtpMutation()
  const [verifyOtp, verifyState] = useVerifyVendorPasswordOtpMutation()
  const [resetPassword, resetState] = useResetVendorPasswordMutation()
  ...
  const submitPassword = async (event) => {
    ...
    try {
      ...
      navigate("/vendor/login", { replace: true, state: { message: response.message } })
    } catch (err) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto flex min-h-[calc(100vh-7rem)] max-w-xl items-center px-3 py-5 sm:px-6 sm:py-10">
        <div className="w-full rounded-2xl border border-border bg-card p-4 shadow-xl sm:p-8">
          <Link to="/vendor/login" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary"><ArrowLeft className="h-4 w-4" />Back to vendor login</Link>
          <div className="mt-6">
            ...
            <h1 className="mt-3 text-2xl font-bold text-foreground sm:text-3xl">Reset vendor password</h1>
            <p className="mt-2 text-sm text-muted-foreground">Use your registered vendor email, verify OTP, then set a new password.</p>
          </div>
            ...
            <form onSubmit={submitEmail} className="mt-6 space-y-4">
              <div>
                ...
                <div className="relative mt-2">
                  ...
                  <Input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="h-12 rounded-xl pl-10" placeholder="vendor@example.com" />
                </div>
              </div>
