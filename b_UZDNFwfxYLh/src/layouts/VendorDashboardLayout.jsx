...
export default function VendorDashboardLayout({ children = <Outlet />, }) {
    ...
    const settings = toDashboardSettings(vendor);
    useEffect(() => {
        ...
        <header className="dashboard-topbar sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border px-4 sm:px-6">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden items-center gap-2 rounded-lg border border-border bg-white/75 px-3 py-2 shadow-sm sm:flex">
              ...
              <input type="text" placeholder={t(settings.appLanguage, "searchBookings")} className="w-48 border-0 bg-transparent text-sm outline-none placeholder:text-muted-foreground"/>
            </div>
          </div>
